...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    def union(that: TweetSet, accu: TweetSet): TweetSet = {
      if (that.isEmpty) accu
      else union(that.tail, accu incl that.head)
    }
    union(that, this)
  }
  // Hint: the method "remove" on TweetSet will be very useful.
  def ascendingByRetweet: Trending = {
    def ascending(tw: TweetSet, tr: Trending): Trending = {
      if(tw.isEmpty) tr
      else{
	      val minTweet = tw.findMin
	      ascending(tw.remove(minTweet),tr+minTweet)
      }
    }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(p=>{ google.exists(x=>{p.text.contains(x)})})

  val appleTweets: TweetSet = TweetReader.allTweets.filter(p=>{ apple.exists(x=>{p.text.contains(x)})})
